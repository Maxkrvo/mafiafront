...
const Container = styled.div<{ $hasHeader?: boolean }>`
  min-height: ${({ $hasHeader }) => $hasHeader ? 'calc(100vh - 70px)' : '100vh'};
  display: flex;
    ...
    return (
      <Container $hasHeader={true}>
        <MainContent>
          ...
          <WelcomeMessage>
            <Subtitle>Welcome back, <PlayerName>{player.nickname}</PlayerName></Subtitle>
            <PlayerRank>Rank: {player.rank} | Reputation: {player.reputation_score}</PlayerRank>
          </WelcomeMessage>
